...
""" Store parse failed api/testcases/testsuites file path
"""
...
def parse_parameters(parameters, variables_mapping=None, functions_mapping=None):
    ...
    Args:
        ...
        variables_mapping (dict): variables mapping loaded from testcases config
...
def _extend_with_testcase(test_dict, testcase_def_dict):
    """ extend test with testcases definition
        test will merge and override testcases config definition.

    Args:
        test_dict (dict): test block
        testcase_def_dict (dict): testcases definition

    Returns:
        dict: extended test dict.

    """
    # override testcases config variables
    ...
    # priority: testcases config > testsuites tests
    ...
    test_name = test_dict.pop("name", None) \
                ...
                or "testcases name undefined"

    # override testcases config name, output, etc.
...
def __prepare_config(config, project_mapping, session_variables_set=None):
    """ parse testcases/testsuites config.
    """
...
def __prepare_testcase_tests(tests, config, project_mapping, session_variables_set=None):
    """ override tests with testcases config variables, base_url and verify.
        test maybe nested testcases.

        variables priority:
        testcases config > testcases test > testcase_def config > testcase_def test > api

        base_url priority:
        testcases test > testcases config > testsuites test > testsuites config > api

        verify priority:
        testcases teststep (api) > testcases config > testsuites config
    ...
    for test_dict in tests:
        ...
        # 1, testcases config => testcases tests
        ...
        if "testcase_def" in test_dict:
            # test_dict is nested testcases

            # pass former teststep's (as a testcases) export value to next teststep
            ...
            # 2, testcases test_dict => testcase_def config
            testcase_def = test_dict.pop("testcase_def")
            _extend_with_testcase(test_dict, testcase_def)

            # verify priority: nested testcases config > testcases config
        ...
        # verify priority: testcases teststep > testcases config
...
def _parse_testcase(testcase, project_mapping, session_variables_set=None):
    ...
    try:
        ...
        logger.error(f"failed to parse testcases: {testcase_path}, error: {ex}")
...
def __get_parsed_testsuite_testcases(testcases, testsuite_config, project_mapping):
    """ override testscases with testsuites config variables, base_url and verify.

        variables priority:
        parameters > testsuites config > testcases config > testcase_def config > testcase_def tests > api

        base_url priority:
        testcase_def tests > testcase_def config > testcases config > testsuites config

    Args:
        testcases (dict):
            {
                "testcase1 name": {
                    "testcases": "testcases/create_user.yml",
                    "weight": 2,
                    "variables": {
                        "uid": 1000
                    },
    ...
    for testcase_name, testcase in testcases.items():
        ...
        # 2, testcases config > testcase_def config
...
def parse_tests(tests_mapping):
        ...
        tests_mapping (dict): project info and testcases list.

            {
                ...
                "testcases": [
                    ...
                    testcase_dict_2     # another testcases dict
                ],
                "api": {
                    "variables": {},
                    "request": {}
    ...
    for test_type in tests_mapping:
        ...
        if test_type == "testsuites":
            ...
            # encapsulate api as a testcases
