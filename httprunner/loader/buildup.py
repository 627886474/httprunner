...
def __extend_with_testcase_ref(raw_testinfo):
    """ extend with testcases reference
    """
    testcase_path = raw_testinfo["testcases"]
    ...
    if testcase_path not in tests_def_mapping["testcases"]:
        ...
        if isinstance(loaded_testcase, list):
            ...
            raise exceptions.FileFormatError(
                f"Invalid format testcases: {testcase_path}")
...
def load_teststep(raw_testinfo):
    """ load testcases step content.
        teststep maybe defined directly, or reference api/testcases.
    ...
    # reference testcases
    elif "testcases" in raw_testinfo:
...
def load_testcase(raw_testcase):
    """ load testcases with api/testcases references.

    Args:
        raw_testcase (list): raw testcases content loaded from JSON/YAML file:
            [
                # config part
                {
                    "config": {
...
def load_testcase_v2(raw_testcase):
    """ load testcases in format version 2.

    Args:
        raw_testcase (dict): raw testcases content loaded from JSON/YAML file:
            {
                "config": {
                    "name": "xxx",
                    "variables": {}
...
def load_testsuite(raw_testsuite):
    """ load testsuites with testcases references.
        support two different formats.

    Args:
        raw_testsuite (dict): raw testsuites content loaded from JSON/YAML file:
    ...
    Returns:
        dict: loaded testsuites content
            {
                "config": {},
    ...
    if isinstance(raw_testcases, dict):
        ...
        # invalid format
        raise exceptions.FileFormatError("Invalid testsuites format!")

    return raw_testsuite


def load_test_file(path):
    """ load test file, file maybe testcases/testsuites/api
...
def load_cases(path, dot_env_path=None):
    """ load testcases from file path, extend and merge with api/testcases definitions.

    Args:
        path (str): testcases/testsuites file/foler path.
    ...
    Returns:
        dict: tests mapping, include project_mapping and testcases.
              each testcases is corresponding to a file.
            {
                "project_mapping": {
                    "PWD": "XXXXX",
                    "functions": {},
                ...
                "testcases": [
                    ...
                    testcase_2_dict     # another testcases dict
                ],
                "testsuites": [
    ...
    def __load_file_content(path):
        ...
        if not loaded_content:
            pass
        elif loaded_content["type"] == "testsuites":
            tests_mapping.setdefault("testsuites", []).append(loaded_content)
        elif loaded_content["type"] == "testcases":
