...
class Runner(object):
    ...
    def __init__(self, config, http_client_session=None):
        """ run testcases or testsuites.

        Args:
            config (dict): testcases/testsuites config dict
    ...
    def _run_testcase(self, testcase_dict):
        ...
        # each teststeps in one testcases (YAML/JSON) share the same session.
