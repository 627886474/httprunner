...
class RunTime:
    ...
    def utcTime(self,days=0,hours=0,sec=0,min=0,zd=False):
        if zd:
            now_time= datetime.date.today() + datetime.timedelta(days=days)
        else:
            now_time = datetime.datetime.now()#+datetime.timedelta(days=days)
        #strToday = datetime.date.today()+ datetime.timedelta(days=days)
        #print(strToday)
        #print(type(strToday.strftime("%Y-%m-%d_%H:%M:%S")))
        d=datetime.datetime.strptime(now_time.strftime("%Y-%m-%d_%H:%M:%S"),"%Y-%m-%d_%H:%M:%S")+ datetime.timedelta(days=days,hours=hours,seconds=sec,minutes=min)
        #print(d)
        utc_time = d - datetime.timedelta(hours=8)  # UTC只是比北京时间提前了8个小时
